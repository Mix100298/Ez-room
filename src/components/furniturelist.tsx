...
export default function FurnitureList({ id , index }: PropsType) {
  const { data, isLoading } = useFetch<Furniture>(
    "http://localhost:5000/api/furnitures/get/" + id
  );
  ...
  return (
    <tr className="mb-2">
      ...
      <img src={data?.image} alt={data?.english_name} className="object-cover aspect-square" />
      </td>
      <td className="max-w-[200px]">
      <a href = {data?.url} target="_blank" className="text-black hover:text-[#60A5FA]" >{data?.english_name}</a>
      </td>
      <td className="">
      {data?.price}
      </td>
